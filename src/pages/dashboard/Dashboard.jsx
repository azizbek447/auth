...
const socialClasses = {
  instagram: 'border-transparent bg-gradient-to-tr from-yellow-400 via-pink-500 to-purple-600 text-white hover:shadow-lg hover:shadow-pink-500/25',
...
function SectionTitle({ eyebrow, title, text, align = 'center', tone = 'light' }) {
  ...
  return (
    <div className={isCenter ? 'mx-auto mb-10 max-w-2xl text-center' : 'mb-8 max-w-2xl'}>
      <span className="mb-3 inline-flex rounded-md bg-sky-50 px-3 py-1 text-xs font-bold uppercase tracking-wide text-sky-700">
        {eyebrow}
      </span>
      <h2 className={`text-4xl font-black leading-tight md:text-5xl ${titleColor}`}>{title}</h2>
...
function Header({ locale, setLocale, t, assets, theme, setTheme, isDark }) {
  ...
  return (
    <header className={`sticky top-0 z-50 border-b backdrop-blur ${isDark ? 'border-white/10 bg-slate-950/88' : 'border-slate-200 bg-white/95'}`}>
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-7 lg:flex">
          {t.nav.map((item) => (
            <a key={item.href} href={item.href} className={`text-sm font-semibold transition hover:text-sky-500 ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>
              {item.label}
            </a>
          ))}
        </nav>
        ...
        <div className="hidden items-center gap-3 lg:flex">
          <div className={`flex rounded-full border p-1 ${isDark ? 'border-white/10 bg-white/5' : 'border-slate-200'}`}>
            {languages.map((item) => (
              <button
                key={item}
                type="button"
                ...
                className={`h-8 rounded-full px-3 text-xs font-black uppercase transition ${
                  locale === item ? 'bg-sky-600 text-white' : isDark ? 'text-slate-300 hover:bg-white/10' : 'text-slate-500 hover:bg-slate-50'
                }`}
              >
                {item}
              </button>
          ...
          <button
            ...
            className={`inline-flex h-10 items-center gap-2 rounded-full border px-4 text-sm font-black transition ${
              isDark ? 'border-white/10 bg-white/5 text-white hover:bg-white/10' : 'border-slate-200 bg-white text-slate-800 hover:border-sky-300'
            }`}
          >
      ...
      {open && (
        <div className={`border-t px-4 py-4 lg:hidden ${isDark ? 'border-white/10 bg-slate-950' : 'border-slate-200 bg-white'}`}>
    ...
    <section className={`overflow-hidden ${isDark ? 'bg-slate-950' : 'bg-white'}`}>
      <div className="mx-auto grid min-h-[calc(100svh-4rem)] max-w-7xl items-center gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[0.88fr_1.12fr] lg:px-8">
        <div className="relative z-10">
          <span className={`mb-5 inline-flex rounded-full px-4 py-2 text-xs font-black uppercase tracking-wide ${isDark ? 'bg-sky-400/10 text-sky-200 ring-1 ring-sky-300/20' : 'bg-sky-50 text-sky-700'}`}>
            {t.hero.eyebrow}
          </span>
          <h1 className={`max-w-3xl text-4xl font-black leading-tight sm:text-5xl lg:text-6xl ${isDark ? 'text-white' : 'text-slate-950'}`}>{t.hero.title}</h1>
          <p className={`mt-6 max-w-2xl text-base leading-8 sm:text-lg ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>{t.hero.text}</p>
          ...
          <dl className="mt-10 grid max-w-3xl grid-cols-2 gap-3 sm:grid-cols-4">
            {stats.map((item) => (
              <div key={item.key} className={`rounded-2xl border p-4 backdrop-blur ${isDark ? 'border-white/10 bg-white/5' : 'border-slate-200 bg-slate-50'}`}>
                <dt className={`text-2xl font-black ${isDark ? 'text-white' : 'text-slate-950'}`}>{item.value}</dt>
                <dd className={`mt-1 text-xs font-semibold leading-5 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{item.label}</dd>
              </div>
            ))}
          </dl>
        </div>
        ...
        <div className="relative">
          <div className={`overflow-hidden rounded-[2.25rem] border shadow-2xl ${isDark ? 'border-white/10 bg-white/5 shadow-black/40' : 'border-white bg-white shadow-slate-200/90'}`}>
            <img
    ...
    <section id="about" className={`py-16 sm:py-20 ${isDark ? 'bg-slate-950' : 'bg-slate-50'}`}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
          <SectionTitle align="left" tone={isDark ? 'dark' : 'light'} eyebrow={t.sections.aboutEyebrow} title={t.sections.aboutTitle} text={t.sections.aboutText} />
          <div className="grid gap-4 sm:grid-cols-2">
            {[
              { icon: FaMicroscope, title: t.sections.servicesTitle, text: t.sections.servicesText },
              { icon: FaUserMd, title: t.sections.doctorsTitle, text: t.sections.doctorsText },
            ].map((item) => {
              const Icon = item.icon;
              return (
                <article key={item.title} className={`rounded-2xl border p-5 shadow-sm ${isDark ? 'border-white/10 bg-white/5' : 'border-slate-200 bg-white'}`}>
                  <Icon className="mb-4 text-2xl text-sky-600" />
                  <h3 className={`font-black ${isDark ? 'text-white' : 'text-slate-950'}`}>{item.title}</h3>
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {stats.map((item) => (
            <div key={item.key} className={`rounded-2xl border p-6 text-center shadow-sm ${isDark ? 'border-white/10 bg-white/5' : 'border-transparent bg-white'}`}>
              <div className="text-3xl font-black text-sky-700">{item.value}</div>
              <div className={`mt-2 text-sm font-bold ${isDark ? 'text-slate-300' : 'text-slate-500'}`}>{item.label}</div>
            </div>
          ))}
        </div>
      </div>
...
function Services({ t, services, isDark }) {
  return (
    <section id="services" className={`py-16 sm:py-20 ${isDark ? 'bg-slate-900' : 'bg-white'}`}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <SectionTitle tone={isDark ? 'dark' : 'light'} eyebrow={t.sections.servicesEyebrow} title={t.sections.servicesTitle} text={t.sections.servicesText} />
            ...
            return (
              <article
                ...
                className={`group overflow-hidden rounded-3xl border shadow-sm transition hover:-translate-y-1 hover:shadow-xl ${
                  isDark ? 'border-white/10 bg-white/5 hover:shadow-black/30' : 'border-slate-200 bg-white hover:shadow-slate-200'
                }`}
              >
                ...
                <div className="p-5">
                  <div className={`mb-4 inline-flex h-11 w-11 items-center justify-center rounded-md ${style}`}>
                    <Icon className="text-xl" />
                  </div>
                  <h3 className={`text-lg font-black ${isDark ? 'text-white' : 'text-slate-950'}`}>{service.title}</h3>
                  <p className={`mt-2 min-h-[72px] text-sm leading-6 ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>{service.desc}</p>
                  <button
                    type="button"
                    onClick={scrollToContact}
...
function Doctors({ t, doctors, isDark }) {
  return (
    <section id="doctors" className={`py-16 sm:py-20 ${isDark ? 'bg-slate-950' : 'bg-slate-50'}`}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <SectionTitle tone={isDark ? 'dark' : 'light'} eyebrow={t.sections.doctorsEyebrow} title={t.sections.doctorsTitle} text={t.sections.doctorsText} />
        ...
        >
          {doctors.map((doctor) => (
            <SwiperSlide key={doctor.key} className="h-auto">
              <article className={`relative h-[500px] overflow-hidden rounded-[2rem] border shadow-xl ${
                isDark ? 'border-white/10 bg-slate-900 shadow-black/30' : 'border-white bg-white shadow-slate-200/90'
              }`}>
                ...
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-slate-950 via-slate-950/86 to-transparent p-5 pt-24">
                  ...
                  <h3 className="text-lg font-black leading-tight text-white">{doctor.name}</h3>
  ...
  return (
    <section id="results" className={`py-16 sm:py-20 ${isDark ? 'bg-slate-900' : 'bg-white'}`}>
      <div className="mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 lg:grid-cols-[0.85fr_1fr] lg:px-8">
        <div>
          <SectionTitle align="left" tone={isDark ? 'dark' : 'light'} eyebrow={t.sections.resultsEyebrow} title={t.sections.resultsTitle} text={t.sections.resultsText} />
          <button
            type="button"
            onClick={scrollToContact}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {t.benefits.map((item, index) => (
            <div key={item} className={`rounded-2xl border p-5 ${isDark ? 'border-white/10 bg-white/5' : 'border-slate-200 bg-slate-50'}`}>
              <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-full bg-sky-600 text-sm font-black text-white">
                {index + 1}
              </div>
              <p className={`text-base font-bold leading-7 ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>{item}</p>
            </div>
          ))}
        </div>
      </div>
    ...
    <section id="locations" className={`py-16 sm:py-20 ${isDark ? 'bg-slate-950' : 'bg-slate-50'}`}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="grid gap-4">
            {branches.map((branch) => {
              ...
              return (
                ...
                >
                  <img src={branch.image} alt={branch.name} className="h-52 w-full rounded-[1.25rem] object-cover object-center sm:h-full" />
                  <span className="flex flex-col justify-center p-2">
                    <span className={`text-2xl font-black ${active || !isDark ? 'text-slate-950' : 'text-white'}`}>{branch.name}</span>
                    <span className={`mt-2 text-sm font-semibold leading-6 ${active || !isDark ? 'text-slate-600' : 'text-slate-300'}`}>
                      {branch.description}
                    </span>
                    <span className={`mt-4 flex items-start gap-2 text-sm font-black ${active ? 'text-sky-700' : isDark ? 'text-sky-300' : 'text-sky-700'}`}>
          ...
          <div className={`overflow-hidden rounded-[2rem] border shadow-2xl ${isDark ? 'border-white/10 bg-white/5 shadow-black/30' : 'border-white bg-white shadow-slate-200/90'}`}>
            <div className="flex flex-wrap items-center justify-between gap-4 p-5">
              <div>
                <h3 className={`text-2xl font-black ${isDark ? 'text-white' : 'text-slate-950'}`}>{activeBranch.name}</h3>
                <p className={`mt-1 text-sm font-semibold ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>{activeBranch.address}</p>
              </div>
              <a
      ...
      <div className="mx-auto grid max-w-7xl gap-8 px-4 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:px-8">
        <div className="rounded-[2rem] bg-slate-950 p-6 text-white shadow-2xl shadow-black/20 sm:p-8">
          ...
          <div className="space-y-4 text-sm">
            <a href={`tel:${clinic.phone}`} className="flex items-start gap-3 rounded-md bg-white/10 p-4 transition hover:bg-white/15">
            ...
            <a href={clinic.mapUrl} target="_blank" rel="noreferrer" className="flex items-start gap-3 rounded-md bg-white/10 p-4 transition hover:bg-white/15">
        ...
        <div className={`rounded-[2rem] border p-6 shadow-xl sm:p-8 ${isDark ? 'border-white/10 bg-white/5 shadow-black/30' : 'border-slate-200 bg-white shadow-slate-200/70'}`}>
          {sent ? (
            <div className="flex min-h-[390px] flex-col items-center justify-center text-center">
              <FaCheckCircle className="mb-5 text-6xl text-emerald-600" />
              <h3 className={`text-2xl font-black ${isDark ? 'text-white' : 'text-slate-950'}`}>{t.common.send}</h3>
              <p className={`mt-3 max-w-md text-sm leading-6 ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>{t.sections.contactText}</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="grid gap-4">
              <label className="grid gap-2">
                <span className={`text-sm font-black ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>{t.common.name}</span>
                <input
                  required
                  type="text"
                  value={form.name}
                  onChange={(event) => setForm((current) => ({ ...current, name: event.target.value }))}
                  className={`h-12 rounded-full border px-4 text-sm outline-none transition focus:border-sky-600 focus:ring-4 focus:ring-sky-100 ${isDark ? 'border-white/10 bg-slate-950 text-white placeholder:text-slate-500' : 'border-slate-300 bg-white'}`}
                  placeholder={t.common.name}
                />
              </label>
              <label className="grid gap-2">
                <span className={`text-sm font-black ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>{t.common.phone}</span>
                <input
                  required
                  type="tel"
                  value={form.phone}
              ...
              <label className="grid gap-2">
                <span className={`text-sm font-black ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>{t.common.department}</span>
                <select
                  required
                  value={form.service}
                  onChange={(event) => setForm((current) => ({ ...current, service: event.target.value }))}
                  className={`h-12 rounded-full border px-4 text-sm outline-none transition focus:border-sky-600 focus:ring-4 focus:ring-sky-100 ${isDark ? 'border-white/10 bg-slate-950 text-white' : 'border-slate-300 bg-white'}`}
                >
    ...
    <footer className={`py-12 ${isDark ? 'bg-black text-white' : 'bg-slate-950 text-white'}`}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid gap-10 lg:grid-cols-[1.2fr_2fr_1fr]">
          ...
          <div>
            <h3 className="text-sm font-black uppercase tracking-wide text-white">{t.footer.contactsTitle}</h3>
            <div className="mt-4 grid gap-4 text-sm text-slate-300">
              ...
              <a href={`mailto:${clinic.email}`} className="flex gap-3 transition hover:text-sky-300">
                <FaEnvelope className="mt-1 shrink-0" />
                <span>{clinic.email}</span>
              </a>
              <a href={clinic.mapUrl} target="_blank" rel="noreferrer" className="flex gap-3 transition hover:text-sky-300">
                <FaMapMarkerAlt className="mt-1 shrink-0" />
...
export default function Dashboard() {
  ...
  return (
    <div className={`min-h-screen font-sans transition-colors ${isDark ? 'bg-slate-950 text-white' : 'bg-white text-slate-900'}`}>
      <style>{`
      ...
      `}</style>
      <Header locale={locale} setLocale={setLocale} t={t} assets={assets} theme={theme} setTheme={setTheme} isDark={isDark} />
      <main>
        ...
        <Locations t={t} branches={branches} selectedBranch={selectedBranch} onBranchSelect={handleBranchSelect} isDark={isDark} />
      ...
      <button
        type="button"
        onClick={scrollToTop}
        className={`fixed bottom-24 right-5 z-50 inline-flex h-12 w-12 items-center justify-center rounded-full border shadow-xl transition hover:-translate-y-1 ${
          isDark ? 'border-white/10 bg-white/10 text-white shadow-black/30 hover:bg-white/15' : 'border-slate-200 bg-white text-slate-800 shadow-slate-300/70 hover:text-sky-700'
        }`}
      ...
      <button
        type="button"
        onClick={scrollToContact}
        className="fixed bottom-5 right-5 z-50 inline-flex h-14 w-14 items-center justify-center rounded-md bg-red-600 text-white shadow-xl shadow-red-950/30 transition hover:bg-red-700"
